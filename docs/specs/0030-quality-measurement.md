...
## 9. 追補：複雑ケースの追加と現状値の実測（2026-06-25）

「単純な合成サンプルしか測れていない」状態を埋めるため、**より複雑な 2 ケース**を追加し、
現状値を実測した（M10-2 のフォローアップ）。

### 追加したもの
- 指標 `table_value_recall(md, values)`：先頭表に各期待値が**位置非依存**で現れた割合。
  結合セル表は平坦化後のセル位置が一意に定まらないため、厳密な位置一致ではなく
  「データが残ったか」を測る。
- サンプル/ケース：
  - `complex_table_sample.pdf` — **結合セル**（列グループ "H1 2026"/"H2 2026" と
    "Region" の行スパン）を持つ 2 段ヘッダ表。指標 `value_recall` ＋ shape。
  - `noisy_scan_sample.pdf` — **劣化スキャン**（傾き 2.3°・ガウスぼかし・加法ノイズ・
    低解像度ラウンドトリップ）。既知テキストに対する `token_recall` で OCR 頑健性を測る。

### 実測値（`PDFTO_RUN_DOCLING_TESTS=1 python -m eval.report`、docling 既定モデル）

| ケース | 指標 | 現状値 | 形状 | 備考 |
|---|---|---|---|---|
| table_doc | cell_recovery | **1.00** | 5×4 | 単純な罫線表。完全復元 |
| complex_table | value_recall | **1.00** | 6×5 | **結合セル表でも全データが残った**（平坦化 6×5） |
| prose | token_recall | **1.00** | — | 本文忠実 |
| scanned（鮮明） | token_recall | **0.75** | — | 4 トークン中 1 つ（複数語/数字）が崩れる |
| noisy_scan（劣化） | token_recall | **1.00** | — | 単語トークンは EasyOCR が頑健に復元 |

A/B（ノブの寄与）:
- `no_cell_match`：complex_table の value_recall に**変化なし**（Δ0）。この合成表では cell
  matching の寄与は出ない。
- `high_res`：noisy_scan に**変化なし**（Δ0）。
- `force_ocr`：noisy_scan は **0.833**（baseline 1.00 から微減）。テキスト層の無いスキャンでは
  強制フルページ OCR の利点は出ず、むしろ僅かに劣化し得る。

**所見:** docling/EasyOCR は今回の合成「複雑表・劣化スキャン」を概ね読み切れている
（結合セルでもデータは残る／単語レベル OCR は頑健）。一方 `scanned` の 0.75 が示すとおり、
複数語・数字列のような**厳しいトークンでは取りこぼし**が出る。今後はより難しいサンプル
（結合セルの厳密位置・多段組み・実写スキャン・数字列）を足して限界点を詰める。
